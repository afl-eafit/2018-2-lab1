...
initDFA :: s -> DFA s a
initDFA q0 = DFA q0 Map.empty Set.empty

trans :: (Ord s, Ord c) => (s, s, c) -> DFA s c -> DFA s c
trans (q, q', c) (DFA q0 ts f) = DFA q0 ts' f
  where ts' = Map.insertWith (Map.union) q (Map.singleton c q') ts

accept :: (Ord a, Ord s) => s -> DFA s a -> DFA s a
accept q (DFA q0 ts fs) = DFA q0 ts (Set.insert q fs)

example :: DFA Int [Char]
example = trans (1, 3, "c") . trans (1, 2, "a") . trans (1, 2, "b") .
  trans (2, 3, "b") . trans (3, 2, "a") . accept 3 $ initDFA 1
